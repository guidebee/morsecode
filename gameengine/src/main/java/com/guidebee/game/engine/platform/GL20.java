...
/**
 * Calls straight through to {@link android.opengl.GLES20} instead of a JNI
 * shim (see jni/Wrapper/AndroidGL20.cpp, now unused/removable) - every call
 * here previously crossed the JNI boundary for no benefit, since Android
 * already provides a direct, low-overhead Java binding. See
 * docs/GAMEENGINE_UPGRADE_PLAN.md Phase 3.0.
 */
public class GL20 implements IGL20 {

    private final int[] ints = new int[1];
    private final int[] ints2 = new int[1];
    private final int[] ints3 = new int[1];
    private final byte[] nameBuffer = new byte[512];
    ...
    public String glGetActiveAttrib(int program, int index, IntBuffer size, Buffer type) {
        GLES20.glGetActiveAttrib(program, index, nameBuffer.length, ints, 0, ints2, 0,
                ints3, 0, nameBuffer, 0);
        size.put(ints2[0]);
        ((IntBuffer) type).put(ints3[0]);
        return new String(nameBuffer, 0, ints[0]);
    }

    public String glGetActiveUniform(int program, int index, IntBuffer size, Buffer type) {
        GLES20.glGetActiveUniform(program, index, nameBuffer.length, ints, 0, ints2, 0,
                ints3, 0, nameBuffer, 0);
        size.put(ints2[0]);
        ((IntBuffer) type).put(ints3[0]);
        return new String(nameBuffer, 0, ints[0]);
    ...
    /**
     * Never called anywhere in this codebase (confirmed by grep) and its
     * signature can't be implemented correctly anyway - a Java String
     * parameter can't be used as an out-param. Kept as a no-op like the
     * upstream libGDX Android backend does for glGetVertexAttribPointerv.
     */
    ...
    /** Not implemented by the upstream libGDX Android backend either. */
