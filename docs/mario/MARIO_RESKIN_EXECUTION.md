...
- [ ] For each, decide Path A vs. Path C (§1) individually rather than batching — a
      ...
      **2026-09-10: Phase 4 done** (`docs/assets/mario-sprites/ampere-staging/build_r4_phase4.py`)
      — `EnemyTurtlePatrol.png` and the FlyingTurtle family (`FlyingTurtle.png`,
      `FlyingTurtledark.png`, `FlyingTurtlePatrol.png`) generated by reusing the Roller
      frames with a minimal thruster-pixel overlay (keeps the silhouette consistent while
      distinguishing the flying variants). Packed successfully: **50 / 122 assets** now
      sourced from the reskin overlay. On-device verification still pending.

      **2026-09-10: Phase 5 done** (`docs/assets/mario-sprites/ampere-staging/build_r4_phase5.py`)
      — turtle shell variants generated from the existing shell art: `TurtelShellRed.png`,
      `TurtelShellFilp.png`, `TurtelShellFilpdark.png`, `TurtelShellFilpRed.png`.
      Packed successfully: **54 / 122 assets** now sourced from the reskin overlay. On-device
      verification still pending.

      **2026-09-10: Phase 6 done** (`docs/assets/mario-sprites/ampere-staging/build_r4_phase6.py`)
      — `FireBall.png` rebuilt from Robot Series `chargedbullet` frames and scaled into
      the required 16×16 frame grid. Packed successfully: **55 / 122 assets** now sourced
      from the reskin overlay. On-device verification still pending.

      **2026-09-10: Phase 7 done** (`docs/assets/mario-sprites/ampere-staging/build_r4_phase7.py`)
      — `Explosion.png` rebuilt from Robot Series `explode` frames and scaled into the
      required 32×32 frame grid. Packed successfully: **56 / 122 assets** now sourced
      from the reskin overlay. On-device verification still pending.

      **2026-09-10: Phase 8 done** (`docs/assets/mario-sprites/ampere-staging/build_r4_phase8.py`)
      — `Lava.png`, `LavaBall.png`, and `Water.png` rebuilt from Kenney's Voxel Pack tiles
      and scaled into the required 32×32 frame grids (lava ball uses a glow pulse for its
      second frame). Packed successfully: **59 / 122 assets** now sourced from the reskin
      overlay. On-device verification still pending.

      **2026-09-10: Phase 9 done** (`docs/assets/mario-sprites/ampere-staging/build_r4_phase9.py`)
      — `Hammer.png` rebuilt from Robot Series bullet-impact frames and scaled into the
      required 28×28 frame grid for `bw_hammer`. Packed successfully: **59 / 122 assets**
      now sourced from the reskin overlay. On-device verification still pending.

      **2026-09-10: Phase 10 done** (`docs/assets/mario-sprites/ampere-staging/build_r4_phase10.py`)
      — `Monkey.png` rebuilt from Robot Series enemy1 frames and scaled into the required
      32×48 frame grid for the 3×2 strip. Packed successfully: **60 / 122 assets** now
      sourced from the reskin overlay. On-device verification still pending.

      **2026-09-10: Phase 11 done** (`docs/assets/mario-sprites/ampere-staging/build_r4_phase11.py`)
      — `FishGrey.png` and `FishRed.png` rebuilt from Robot Series flying-effect frames,
