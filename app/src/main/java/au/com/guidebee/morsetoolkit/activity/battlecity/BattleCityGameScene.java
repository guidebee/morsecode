...
import au.com.guidebee.morsetoolkit.activity.battlecity.actors.*;
...
public class BattleCityGameScene extends ScreenAdapter implements GameControllerListener {
     ...
     */
    public static TextureRegion imgNumberBlack =
            resourceManager.getImage(ResourceManager.NUMBER_BLACK);

    /**
     * enemy icon
     */
    public static TextureRegion imgEnemyIcon =
            resourceManager.getImage(ResourceManager.ENEMY_ICON);

    /**
     * first player icon
     */
    public static TextureRegion imgIP =
            resourceManager.getImage(ResourceManager.IP);

    /**
     * flag image
     */
    public static TextureRegion imgFlag =
    ...
    private final int gameWorldWidth = 420;
    private final int gameWoldHeight = 240;
    ...
    public BattleCityGameScene() {
        ...
        if (battleField == null) {
            ...
            battleField.readBattlefieldFromLedLetter();
    ...
    public void newGame() {
        ...
        playerTank.initTank();
        playerTank.setAvailableLives(DEFAULT_PLAYER_LIVE);
        EnemyTank.explodeAllEnemies();
    ...
    private void putAnPowerup() {
        int powerupSelection = Math.abs(rnd.nextInt()) % 100;
        int type;
        if (powerupSelection > 95) {
            type = Powerup.TANK;
        } else if (powerupSelection > 80) {
            type = Powerup.BOMB;
        } else if (powerupSelection > 70) {
            type = Powerup.SHOVEL;
        } else if (powerupSelection > 60) {
            type = Powerup.CLOCK;
    ...
    private void applyGameLogic() {
        ...
        if (!isGameover) {
            ...
            if (!playerTank.isVisible()) {
                if (playerTank.getAvailableLives() > 0) {
                    playerTank.initTank();
    ...
    private void drawScoreBar(Batch g){
        ...
        for(int i=0;i<enemyTankRemains-EnemyTank.getVisibleEnemyTanks();i++){
            ...
            x=4+(i %10) *imgEnemyIcon.getRegionWidth();
            y=sceneHeight-(marginY+changeRow*imgEnemyIcon.getRegionWidth());
            g.draw(imgEnemyIcon, x, y);
        }
        //draw IP
        x=sceneWidth-imgEnemyIcon.getRegionWidth()*10+offset;
        y=sceneHeight-marginY;
        int lives=playerTank.getAvailableLives();
