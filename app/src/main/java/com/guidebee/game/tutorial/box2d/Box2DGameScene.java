...
public class Box2DGameScene extends Scene {
    ...
    public Box2DGameScene(String stageClass){
        super(createStage(stageClass));

    }

    private static Stage createStage(String stageClass) {
        try {
            return (Stage) Class.forName(stageClass).getDeclaredConstructor().newInstance();
        } catch (ReflectiveOperationException e) {
            throw new RuntimeException("Unable to instantiate Box2D tutorial stage: " + stageClass, e);
        }
    }
