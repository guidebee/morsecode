...
import au.com.guidebee.morsetoolkit.activity.battlecity.BattleCityGameScene;
...
public abstract class EnemyTank extends Tank {
     ...
     */
    public static void explodeAllEnemies() {
     ...
     * Player has collected {@see Powerup.BOMB}. Enemies should explode immediately.
     */
    public static void explodeNearHomeEnemies() {
        int x = (battleField.WIDTH_IN_TILES / 2 - 3) * ResourceManager.TILE_WIDTH / 2 - ResourceManager.TILE_WIDTH;
        int y = 0;
        Rectangle rectangle=new Rectangle(x-50,y-50,100,100);
        for (int i = 1; i < POOL_SIZE; ++i) {
            Tank tank = TANK_POOL[i];
            if (tank.isVisible()) {
                if(rectangle.contains(tank.getCenterX(),tank.getCenterY())) {
                    ((EnemyTank) tank).blood = 1;
                    tank.explode();
                }
            }
    ...
    public void explode() {
        ...
        if (type == TYPE_HEAVY && hasPrize && blood == 4) {
            BattleCityGameScene.canPutPowerup = true;
            hasPrize = false;
        }
        blood--;
        if (blood == 0) {
            ...
            BattleCityGameScene.enemyTankRemains--;
            if (hasPrize) {
                BattleCityGameScene.canPutPowerup = true;
            }
            BattleCityGameScene.enemyTanksCount[type]++;
        }
    }
